...
//Saco aquellas naves cuyos pasajeros sean mayor que 100
let arrNav = naves.results;
let passMore100 = arrNav.filter(x => x.passengers > 100);
let modelName = passMore100.map(x => ({ name: x.name, model: x.model }));
console.log("modelName", modelName);

//Forma uno con la funcion incrustada en el filter
let passLess100 = arrNav.filter(x => {
    let valor = isNaN(x.passengers) ? 0 : parseInt(x.passengers);
    if (valor <= 100) {
        return true
    } else {
        return false
...
console.log("passLess100", passLess100);

//Forma dos con un or dentro del filter
let passLess100B = arrNav.filter(x => (parseInt(x.passengers) <= 100 || x.passengers == "unknown"));
console.log("passLess100B", passLess100B);

//Funcion externa
let passLess100C = arrNav.filter(x => comprobarNumero(x.passengers));
console.log("passLess100C", passLess100C);
